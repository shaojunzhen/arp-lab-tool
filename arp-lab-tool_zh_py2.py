...
def select_targets(devices):
    ...
    while True:
        choice = raw_input(f"{bcolors.BOLD}[*] 输入选择（直接回车选择全部）：{bcolors.ENDC}")
        if not choice:
...
def enable_ip_forward():
    if sys.platform.startswith("linux"):
        ...
        print("3. 启动后受害者可正常上网（需管理员权限）{bcolors.ENDC}")
...
def traffic_monitor():
    while not restore_flag:
        ...
        print(f"\r{bcolors.BOLD}[流量监控] 上传：{upload_mb:.2f}MB | 下载：{download_mb:.2f}MB | 抓包数：{capture_count}{bcolors.ENDC}"),
        sys.stdout.flush()
        time.sleep(1)
...
def main():
    ...
    mode = raw_input(f"{bcolors.BOLD}[*] 选择模式（1/2，默认1）：{bcolors.ENDC}") or "1"
    ...
    confirm = raw_input(f"{bcolors.BOLD}[*] 确认开始？（y/n）：{bcolors.ENDC}").lower()
    if confirm != "y":
    ...
    try:
        while True:
            ...
            print(f"\r{bcolors.BOLD}[*] 已发送 {packet_count} 个包 | 持续时间：{elapsed:.1f} 秒{bcolors.ENDC}"),
            sys.stdout.flush()
            time.sleep(0.5)
