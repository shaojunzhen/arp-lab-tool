...
def select_targets(devices):
    ...
    while True:
        choice = raw_input(f"{bcolors.BOLD}[*] Enter Selection (Press Enter to select all)：{bcolors.ENDC}")
        if not choice:
...
def enable_ip_forward():
    if sys.platform.startswith("linux"):
        ...
        print("3. Victims Can Access Internet After Starting (Need Admin Privileges){bcolors.ENDC}")
...
def traffic_monitor():
    while not restore_flag:
        ...
        print(f"\r{bcolors.BOLD}[Traffic Monitor] Upload：{upload_mb:.2f}MB | Download：{download_mb:.2f}MB | Capture Count：{capture_count}{bcolors.ENDC}"),
        sys.stdout.flush()
        time.sleep(1)
...
def main():
    ...
    mode = raw_input(f"{bcolors.BOLD}[*] Select Mode (1/2, Default 1)：{bcolors.ENDC}") or "1"
    ...
    confirm = raw_input(f"{bcolors.BOLD}[*] Confirm to Start？(y/n)：{bcolors.ENDC}").lower()
    if confirm != "y":
    ...
    try:
        while True:
            ...
            print(f"\r{bcolors.BOLD}[*] Sent {packet_count} Packets | Duration：{elapsed:.1f} Seconds{bcolors.ENDC}"),
            sys.stdout.flush()
            time.sleep(0.5)
